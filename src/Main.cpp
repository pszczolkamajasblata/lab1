...
public:
	...
	void Draw(Color col) const {
		for (size_t i = 0; i < history.size(); ++i) {
			...
			DrawCircleV(history[i], 2, Fade(col, alpha));
		}
	}
...
public:
	HeartAsteroid(int w, int h) : Asteroid(w, h) { baseDamage = 20; }
...
private:
	void DrawHeart(Vector2 center, float size, float rotation) const {
		const int points = 100;
		Vector2 shape[points];
		...
		for (int i = 0; i < points; i++) {
			...
			Vector2 p = { x, y };
			p = Vector2Rotate(p, DEG2RAD * rotation);
			shape[i] = Vector2Add(center, p);
		}

		for (int i = 0; i < points - 1; i++) {
			DrawLineV(shape[i], shape[i + 1], RED);
		}
		DrawLineV(shape[points - 1], shape[0], RED);
	}
};
...
public:
	PlayerShip(int w, int h) : Ship(w, h) {
		texture = LoadTexture("assets/spaceship1.png");
		...
		while (!WindowShouldClose()) {
			...
			// Render everything
			{
				...
				if (!player->IsAlive()) {
					...
					DrawText(msg, (C_WIDTH - textWidth) / 2, C_HEIGHT / 2 - 20, fontSize, RED);
				}
